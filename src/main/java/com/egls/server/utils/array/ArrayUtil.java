 ...
 * 提供一些数组的工具方法
 *
 * @author mayer - [Created on 2018-08-09 21:12]
 */
public final class ArrayUtil {

    public static <S, T> void cast(final S[] original, final T[] target, final Caster<S, T> caster) {
        for (int i = 0; i < original.length; i++) {
            target[i] = caster.cast(original[i]);
        }
    }
    ...
    /**
     * 与直接使用Arrays.copyOf不同,当复制的范围超过源数组,返回的数组长度不会超过源数组长度.
     *
     * @param array  源数组
     * @param length 拷贝长度
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static <E> E[] copyOf(final E[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static long[] copyOf(final long[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static int[] copyOf(final int[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static short[] copyOf(final short[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static char[] copyOf(final char[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static byte[] copyOf(final byte[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static double[] copyOf(final double[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static float[] copyOf(final float[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }
     ...
     * @return 数组拷贝的定长结果, 最长不会超过源数组
     */
    public static boolean[] copyOf(final boolean[] array, final int length) {
        return Arrays.copyOf(array, Math.min(length, array.length));
    }

    /**
     * 与直接使用Arrays.copyOfRange不同,当复制的范围超过源数组,返回的数组长度不会超过源数组长度.
     *
     * @param original 源数组
     * @param from     起始索引
     * @param to       结束索引
     * @return 数组拷贝的定长结果, 最长不会超过源数组, 结果包含起始索引元素, 不包含结束索引元素
     */
    public static <E> E[] copyOfRange(final E[] original, final int from, final int to) {
        return Arrays.copyOfRange(original, from, Math.min(to, original.length));
    }
    ...
    public static <E> void shuffle(final E[] array) {
        for (int i = 1; i < array.length; i++) {
            swap(array, i, RandomUtil.randomInt(array.length));
        }
    }
    ...
    public static boolean[] toBooleanPrimitiveArray(final String[] strings) {
        ...
        for (int index = 0; index < strings.length; index++) {
            try {
                result[index] = StringUtil.toBoolean(strings[index]);
            } catch (Exception exception) {
                throw new RuntimeException(String.format("[index:%d] -> %s", index, exception.getMessage()), exception);
            }
        }
